...
const Notification = () => {
  const { currentColor,isClicked,setIsClicked, screenSize} = useStateContext();
  const handleClosebtn = () => {
    if (isClicked !== undefined && screenSize <= 900) {
      setIsClicked(false);
    }
  };
  return (
    <div className="nav-item absolute right-5 md:right-40 top-16 bg-white dark:bg-[#42464D] p-8 rounded-lg w-96">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setIsClicked(!isClicked)}><MdOutlineCancel /></button>
      </div>
      <div className="mt-5 ">
